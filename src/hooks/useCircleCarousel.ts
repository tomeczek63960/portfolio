...
export const useCircleCarousel = (speed: number = 800, autoplay: number = 4500): [
  ...
  useIsomorphicLayoutEffect((): void => {
    const dots = pagination.current?.children as HTMLCollectionOf<HTMLElement> | never[];
    const slides = carouselTextRef.current?.children as HTMLCollectionOf<HTMLElement> | never[];
    configObject.current = {
      ...configObject.current,
      slides: slides,
      dots: dots,
      step: -360 / (dots?.length || 0),
    }

    Array.from(dots).forEach((dot: HTMLElement, index: number) => {
      dot.style.transform = `rotate(${360 / dots.length * index}deg)`;
    });
    if (pagination.current) pagination.current.style.transitionDuration = `${speed}ms`;
    if (autoplay && !configObject.current.autoplayId) startAutoplay();
  }, []);
  ...
  const getTransformAnimateObject = (type: string = 'in'): any => { // TODO: dodać typ zwracanego obiektu
    let animationObject;
    if(type === 'in') {
      animationObject = {
        duration: 0.3,
  ...
  const getTransformSvgObject = (type: string = 'in'): any => { // TODO: dodać typ zwracanego obiektu
    let animationObject;
    if(type === 'out') {
      animationObject = {
        duration: 0.5,
  ...
  const findClickedElementIndex = (event: React.MouseEvent<HTMLDivElement, MouseEvent>): number => {
    let target = event.target as HTMLElement;
    while (!target.dataset.carouselItem) {
      if (!target.parentElement) break;
      target = target.parentElement;
    }
    return Array.from(configObject.current.dots).findIndex((item: HTMLElement) => item === target);
  }

  const changeSlideAnimation = (index: number, classAction: 'add' | 'remove'): void => {
    ...
    slideTimeline.current.to(svg, getTransformSvgObject(isEnterAnimation ? 'in' : 'out'), 'start');
    slideTimeline.current.to(slide.children, getTransformAnimateObject(isEnterAnimation ? 'in' : 'out'));
  }

  const setSlide = (event: React.MouseEvent<HTMLDivElement, MouseEvent> | number): void => {
    const prev = configObject.current.activeSlide;
    const index = typeof event === 'number' ? event : findClickedElementIndex(event);
    ...
    configObject.current.angle += getAngle(configObject.current.dots.length, active, prev, configObject.current.step);
