...
const _bonusValidated = 1;
const _getComputedStyle = (element) => _win.getComputedStyle(element);
const _isArray = Array.isArray;
const _slice = [].slice;
const _toArray = (value, leaveStrings) => {
  let type;
...
const _isAbsolute = (vars) =>
  vars.position === "absolute" || vars.absolute === true;
const _findSpecialChars = (text, chars) => {
  let i = chars.length;
  let s;
  while (--i > -1) {
    s = chars[i];
...
const _divStart = " style='position:relative;display:inline-block;'";
const _cssClassFunc = (cssClass = "", tag) => {
...
const _swapText = (element, oldText, newText) => {
...
const _pushReversed = (a, merge) => {
  let i = merge.length;
  while (--i > -1) {
    a.push(merge[i]);
  }
};
const _isBeforeWordDelimiter = (e, root, wordDelimiter) => {
  let next;
  while (e && e !== root) {
...
const _deWordify = (e) => {
  const children = _toArray(e.childNodes);
...
const _getStyleAsNumber = (name, computedStyle) =>
  parseFloat(computedStyle[name]) || 0;
const _setPositionsAfterSplit = (
  element,
  vars,
  allChars,
  allWords,
  ...
  const paddingLeft = _getStyleAsNumber("paddingLeft", cs);
  let lineOffsetY = -999;
  ...
  const lineThreshold = _getStyleAsNumber("fontSize", cs) * 0.2;
  ...
  const absolute = _isAbsolute(vars);
  let linesClass = vars.linesClass;
...
const _splitRawText = (element, vars, wordStart, charStart) => {
  const tag = vars.tag ? vars.tag : vars.span ? "span" : "div";
  const types = vars.type || vars.split || "chars,words,lines";
  ...
  const hasTagStart = text.indexOf("<") !== -1;
  ...
  const l = text.length;
  HandleText = (text.charAt(0) === " " ? space : "") + wordStart();
  for (i = 0; i < l; i++) {
  ...
  element.outerHTML = HandleText + (wordIsOpen ? wordEnd : "");
  if (hasTagStart) {
    _swapText(parent, "{{LT}}", "<"); // note: don't perform this on "element" because that gets replaced with all new elements when we set element.outerHTML.
  }
};
const _split = (element, vars, wordStart, charStart) => {
  const children = _toArray(element.childNodes);
  const l = children.length;
  const absolute = _isAbsolute(vars);
  let i;
  let child;
...
export class HandleText {
  ...
  static create(element, vars) {
    return new HandleText(element, vars);
  }
}

export { HandleText as default };
